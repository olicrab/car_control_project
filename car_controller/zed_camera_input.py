"""Источник команд от ZED-камеры: инициализирует стереокамеру, получает
изображение и глубину, по расстоянию до препятствий
решает, двигаться вперёд или тормозить, возвращая команды в формате
InputDevice. Опционально, ведёт запись видео"""
