...
def curses_interface(stdscr, controller, gamepad_input, camera_input, is_gamepad_mode):
    ...
    stdscr.addstr(12, 0, f"Position (x,y,z): {camera_input.translation}")
    stdscr.addstr(13, 0, f"Rotation (r,p,y): {camera_input.rotation}")
    stdscr.addstr(14, 0, f"Tracking: {camera_input.tracking}")
    stdscr.addstr(16, 0, f"Last Error: {camera_input.last_error or 'None'}")
    stdscr.addstr(18, 0, "Left Stick: steering, Triggers: throttle/brake, Bumpers: gears")
    stdscr.addstr(19, 0, "Start: mode, A: record, B: reverse, X: reset trim, Y: reset depth")
    stdscr.addstr(20, 0, "D-Pad: trim (left/right: ±2), depth (up/down: ±0.05)")
    stdscr.addstr(21, 0, "Q: exit")
